...
class Command(BaseCommand):
    help = 'Deletes all rows from database tables backing the floodfilling models.'
    ...
    def handle(self, *args, **options):
        ...
        while selection.lower() not in ['y', 'n', '']:
            selection = input('This will empty all floodfilling-related database tables. Are you sure ([y]/n)? ')
        ...
        for model in apps.get_app_config('floodfilling').get_models():
        ...
        self.stdout.write(self.style.SUCCESS('Successfully cleared floodfilling tables'))
