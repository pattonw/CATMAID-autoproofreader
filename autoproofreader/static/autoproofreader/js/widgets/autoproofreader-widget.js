...
(function (CATMAID) {
    ...
    */
    ...
    AutoproofreaderWidget.prototype.getProofreaderSegmentationLayerOptions = function () {
        let self = this;
        return CATMAID.fetch("ext/autoproofreader/" + project.id + "/autoproofreader-results", "GET", { result_id: self.ranking_result_id }).then((result) => {
            console.log(result);
            return CATMAID.fetch("ext/autoproofreader/" + project.id + "/autoproofreader-results", "GET", { result_id: self.ranking_result_id, uuid: true }).then((uuid) => {
                console.log(uuid);
                return CATMAID.fetch("files/proofreading_segmentations/" + uuid + "/segmentations.n5/segmentation_counts/attributes.json", "GET").then((attrs_file) => {
                    ...
                    var options = {
                        ...
                        stack_attrs: {
                            dimensions: { x: attrs_file.dimensions[0], y: attrs_file.dimensions[1], z: attrs_file.dimensions[2] },
                            scales: [{ x: 1, y: 1, z: 1 }],
                            ...
                            tile_source_type: 11,
                            image_base: "http://localhost:8000/files/proofreading_segmentations/" + uuid + "/segmentations.n5/segmentation_counts/0_1_2"
                        }
                    };
                    return options
                })
        ...
        this.getProofreaderSegmentationLayerOptions().then((options) => {
            ...
            project.getStackViewers().forEach(function (sv) {
                ...
                if (options.visible) {
                    if (!layer) {
                        // Create new if not already present
                        let stack = new CATMAID.Stack('seg', 'Segmenations',
                            options.stack_attrs.dimensions,
                            options.stack_attrs.resolution,
                            { x: 0, y: 0, z: 0 },
                            [],
                            options.stack_attrs.scales, -2, '', '', '', 0,
                            { x: 0, y: 0, z: 0 }, [0, 0, 0],
                            [
                                {
                                    id: 'no-mirror-id',
                        ...
                        layer = new CATMAID.ProofreadSegmentationLayer(options, sv, "segmenation layer", stack, 0, true, 1, false, "inherit", false, true);
