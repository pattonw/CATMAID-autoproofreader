...
(function(CATMAID) {
  ...
  var ProofreadSkeletonVisualizationLayer = function(stackViewer, options) {
    ...
    if (initialSource) {
      ...
      CATMAID.Error(
        "Proofread skeleton visualizer layer needs a result to visualize!"
      );
    }
   ...
   */
  ProofreadSkeletonVisualizationLayer.updateDefaultOptions = function(options) {
  ...
  ProofreadSkeletonVisualizationLayer.prototype._createProjection = function(
    ...
    var renderOptions = {
      ...
      color: material.color(
        ...
        this.options.uColor,
        this.options.dColor,
        this.options.pColorC,
        this.options.pColorD,
        this.options.bColor
      ),
  ...
  function renderSloppedEdge(
    ...
    let ndistabove =
      above[options.normalDim] - options.stackViewer.plane.constant;
    let ndistbelow =
      below[options.normalDim] - options.stackViewer.plane.constant;
    if (ndistabove < ndistbelow) {
      CATMAID.Error(
        "Something went wrong with rendering the proofread skeleton!"
      );
    }
    let abs_slope = {
      x: above.x - below.x,
    ...
    let slope = {
      x: abs_slope.x / abs_slope.z,
      y: abs_slope.y / abs_slope.z,
      z: abs_slope.z / abs_slope.z
    };
    let plane_intercept = {
      x: above.x - ndistabove * slope.x,
      y: above.y - ndistabove * slope.y,
      z: above.z - ndistabove * slope.z
    };
    ...
    let ndista = Math.min(
      ndistabove,
      options.stackViewer.primaryStack.resolution[options.normalDim]
    );
    let ndistb = Math.max(
      ndistbelow,
      -options.stackViewer.primaryStack.resolution[options.normalDim]
    ...
    if (ndista > 0) {
      let up_line = {
        x: plane_intercept.x + ndista * slope.x,
        y: plane_intercept.y + ndista * slope.y,
        z: plane_intercept.z + ndista * slope.z
      };
      let color = options.color(
        n,
        up_line,
        up_line[options.normalDim],
        !parent_first,
        connectivity,
        branch
      );
      ...
      edge.lineStyle(options.edgeWidth, 0xffffff, opacity);
      edge.moveTo(
        bottom_edge[options.planeDims.x],
        bottom_edge[options.planeDims.y]
      );
    ...
    if (ndistb < 0) {
      let down_line = {
        x: plane_intercept.x + ndistb * slope.x,
        y: plane_intercept.y + ndistb * slope.y,
        z: plane_intercept.z + ndistb * slope.z
      };
      let color = options.color(
        n,
        down_line,
        down_line[options.normalDim],
        parent_first,
        connectivity,
        branch
      );
      ...
      edge.lineTo(
        down_line[options.planeDims.x],
        down_line[options.planeDims.y]
      );
      edge.tint = color;
      options.graphics.containers.lines.addChild(edge);
    }
  ...
  function renderNodes(n, i, nodes) {
    ...
    if (e) {
      ...
      let connectivity =
        this.node_map[n].connectivity_score / this.max_connectivity_score;
  ...
  ProofreadSkeletonVisualizationLayer.shadingModes = {
    ...
    skeletoncolorgradient: {
      ...
      color: function(
        ...
        up_color,
        down_color,
        parent_color_connected,
        parent_color_disconnected,
        branch_color
      ) {
        ...
        return function(node, pos, z, p, connectivity, b) {
          ...
          if (p) {
            return (
              (((p_color_c.r * connectivity +
                p_color_d.r * (1 - connectivity)) *
                255) <<
                16) +
              (((p_color_c.g * connectivity +
                p_color_d.g * (1 - connectivity)) *
                255) <<
                8) +
              (p_color_c.b * connectivity + p_color_d.b * (1 - connectivity)) *
                255
            );
          }
          if (b) {
